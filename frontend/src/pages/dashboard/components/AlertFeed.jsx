export default function AlertFeed({ recentEvents }) {
  const typeStyles = {
    anomaly: { color: '#D32F2F', bg: '#FFEBEE' },
  ...
  return (
    <div className="md:col-span-12 lg:col-span-4 bg-white rounded-xl border border-[#E5E7EB] flex flex-col shadow-sm">
      <div className="px-6 py-4 border-b border-[#E5E7EB]">
        <h3 className="text-[15px] font-semibold text-[#1B1D1B]">Alert Feed</h3>
      </div>
      {recentEvents.length === 0 ? (
        ...
        <ul className="divide-y divide-[#E5E7EB] overflow-y-auto max-h-[320px]">
          {recentEvents.map((alert, i) => {
            ...
            return (
              <li key={i} className="px-6 py-4 hover:bg-[#F8FAF7] transition-colors flex gap-3.5">
                <span
                ...
                <div>
                  <p className="text-sm font-semibold text-[#1B1D1B] leading-snug">{alert.title}</p>
