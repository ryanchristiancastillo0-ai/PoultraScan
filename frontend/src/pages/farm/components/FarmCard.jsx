...
export default function FarmCard({ farm, isActive, onSelectActive, onOpenDetails }) {
  return (
    <div
      className={`group relative bg-white rounded-xl border overflow-hidden flex flex-col
                  ...
                  ${
                    isActive
                      ? 'border-[#2E7D32] shadow-[0_4px_20px_rgba(46,125,50,0.12)]'
                      : 'border-[#E5E7EB] shadow-sm hover:border-[#D1D5DB] hover:shadow-[0_8px_24px_rgba(0,0,0,0.06)] hover:-translate-y-0.5'
                  }`}
    >
      <button
        onClick={onOpenDetails}
        className="w-full aspect-[16/10] overflow-hidden relative text-left bg-[#E8F5E9]"
      ...
      >
        {farm.image_url ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#E8F5E9] to-[#E8F5E9]">
  ...
  {/* backdrop circle */}
  <circle cx="32" cy="32" r="30" fill="#E8F5E9" stroke="#2E7D32" strokeWidth="1.5" />

  {/* placeholder image frame */}
  <rect x="18" y="20" width="28" height="24" rx="4" stroke="#2E7D32" strokeWidth="2" fill="none" />

  {/* sun / focus accent */}
  <circle cx="25" cy="27" r="2.5" fill="#2E7D32" />
  ...
  />
        ...
        <div className="absolute top-3 left-3 bg-white/95 backdrop-blur-sm text-[#2E7D32] text-[11px] font-semibold uppercase tracking-wide px-2.5 py-1 rounded-full flex items-center gap-1.5 shadow-sm">
          <span className="w-1.5 h-1.5 rounded-full bg-[#2E7D32]" />
          Active
        </div>
        {isActive && (
          <div className="absolute top-3 right-3 bg-[#2E7D32] text-white text-[11px] font-semibold uppercase tracking-wide px-2.5 py-1 rounded-full flex items-center gap-1 shadow-sm">
      ...
      <div className="p-5 flex flex-col flex-grow">
        <div className="flex items-start justify-between gap-2">
          <h3 className="text-base font-bold text-[#1B1D1B] line-clamp-1 tracking-tight">{farm.farm_name}</h3>
          <button
            onClick={onOpenDetails}
            className="text-[#6B7280] hover:text-[#2E7D32] p-0.5 rounded-md transition-all duration-200 hover:translate-x-0.5"
        ...
        <div className="mt-5 pt-4 border-t border-[#E5E7EB] flex items-center justify-between gap-4">
          <div className="flex flex-col">
            ...
            <span className="text-sm font-bold text-[#1B1D1B] tabular-nums">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 ${
              isActive
                ...
                : 'bg-[#F8FAF7] text-[#6B7280] hover:text-[#1B1D1B] hover:bg-[#E5E7EB]'
            }`}
          >
            {isActive ? (
              <>
