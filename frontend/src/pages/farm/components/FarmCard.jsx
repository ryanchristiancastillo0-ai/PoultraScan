...
export default function FarmCard({ farm, isActive, onSelectActive, onOpenDetails }) {
  return (
    <div
      className={`group relative bg-white rounded-xl border overflow-hidden flex flex-col shadow-sm transition-colors duration-200 ${
        isActive ? 'border-[#2F5D3A]' : 'border-[#E5E7EB] hover:border-[#D1D5DB]'
      }`}
    >
      <button
  ...
  <svg viewBox="0 0 64 64" className="w-16 h-16" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {/* tail feather */}
    <path
      d="M16 34c-4-2-6-6-5-10 3 1 6 4 7 8z"
      fill="#2F5D3A"
    />
    ...
    {/* wing */}
    <path
      d="M22 33c3-1 7-1 9 2-3 3-7 3-10 1z"
      fill="#274d31"
    />
    ...
    <path d="M23 54h6M31 54h6" stroke="#F59E0B" strokeWidth="2" strokeLinecap="round" />
        ...
        {isActive && (
          <div className="absolute top-3 right-3 bg-[#2F5D3A] text-white text-[11px] font-semibold uppercase tracking-wide px-2.5 py-1 rounded-full flex items-center gap-1">
      ...
      <div className="p-5 flex flex-col flex-grow">
        <div className="flex items-start justify-between gap-2">
          <h3 className="text-base font-bold text-[#111827] line-clamp-1">{farm.farm_name}</h3>
          <button 
            onClick={onOpenDetails}
            className="text-[#6B7280] hover:text-[#2F5D3A] p-0.5 transition-colors"
