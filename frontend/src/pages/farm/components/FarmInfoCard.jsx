...
export default function FarmInfoCard({ farmName, location, capacity, imageUrl }) {
  ...
  return (
    <div className="lg:col-span-8 bg-white rounded-2xl border border-[#E5E7EB] shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden flex flex-col sm:flex-row">
      <div className="w-full sm:w-2/5 aspect-[4/3] sm:aspect-auto relative flex-shrink-0 bg-[#E8F5E9] overflow-hidden group">
        {imageUrl ? (
          <img
            alt={farmName || 'Farm'}
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-[#E8F5E9] to-[#E8F5E9]">
            <svg viewBox="0 0 64 64" className="w-16 h-16 text-[#2E7D32]" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <path
                ...
                strokeLinecap="round"
              />
              <circle cx="32" cy="24" r="8" fill="#2E7D32" />
              <path
        ...
        <div className="absolute top-3 left-3 bg-white/95 backdrop-blur-sm text-[#2E7D32] text-[11px] font-semibold uppercase tracking-wide px-2.5 py-1 rounded-full flex items-center gap-1.5 shadow-sm ring-1 ring-black/5">
      ...
      <div className="flex flex-col justify-center w-full p-5 md:p-7">
        ...
        <h2 className="text-2xl font-bold text-[#1B1D1B] mb-5 tracking-tight leading-snug">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {details.map(({ icon: Icon, label, value }, i) => (
            <div
              key={i}
              className="flex items-start gap-3 rounded-xl p-2.5 -m-2.5 transition-colors hover:bg-[#F8FAF7]"
            >
              <span className="w-10 h-10 rounded-xl bg-[#E8F5E9] text-[#2E7D32] flex items-center justify-center flex-shrink-0 ring-1 ring-[#2E7D32]/10">
                <Icon className="text-lg" />
              </span>
              <div className="min-w-0 pt-0.5">
                <p className="text-[11px] text-[#6B7280] uppercase tracking-wide font-semibold">{label}</p>
                <p className="text-[15px] text-[#1B1D1B] font-bold mt-0.5 truncate">{value}</p>
              </div>
            </div>
          ))}
        </div>
