...
export default function ResetPassword() {
  ...
  const evaluatePasswordStrength = (pass) => {
    if (!pass) return { score: 0, text: 'Very Weak', color: 'bg-[#E5E7EB]' };
    ...
    switch (score) {
      case 1:
        return { score: 1, text: 'Weak', color: 'bg-[#D32F2F]' };
      case 2:
        return { score: 2, text: 'Fair', color: 'bg-[#ffb95f]' };
      case 3:
        return { score: 3, text: 'Good', color: 'bg-[#43A047]' };
      case 4:
        return { score: 4, text: 'Strong', color: 'bg-[#43A047]' };
      default:
        return { score: 0, text: 'Very Weak', color: 'bg-[#D32F2F]' };
    }
  };
  ...
  return (
    <div className="bg-[#F8FAF7] text-[#1B1D1B] antialiased min-h-[100dvh] flex flex-col justify-between w-full">

      {/* Main Container */}
      <main className="flex-grow flex items-center justify-center p-4 sm:p-8 md:p-12">
        <div className="w-full max-w-[440px] bg-white rounded-2xl sm:rounded-[32px] p-6 sm:p-10 md:p-12 shadow-[0_20px_50px_-12px_rgba(0,105,72,0.12)] border border-[#D8E3DA]/20 flex flex-col gap-6 sm:gap-9 my-auto">
          ...
          {!isSuccess ? (
            <>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-5 sm:gap-6">
                  ...
                  <div className="relative flex items-center h-[52px] sm:h-[58px] bg-[#F8FAF7]/60 border-2 border-[#D8E3DA]/60 rounded-xl sm:rounded-2xl focus-within:border-[#2E7D32] focus-within:bg-white transition-all duration-300 overflow-hidden px-4 sm:px-5">
                    ...
                    <label
                      ...
                      className={`absolute left-11 sm:left-14 origin-left pointer-events-none transition-all duration-250 ease-[cubic-bezier(0.25,0.8,0.25,1)] ${
                        passwordFloated
                          ? 'top-1.5 scale-[0.8] font-semibold text-[#2E7D32]'
                ...
                {!passwordsMatch && (
                  <p className="text-xs text-[#D32F2F] font-semibold -mt-2 text-left px-1 animate-[fadeIn_0.2s_ease-out]">
                ...
                <div className="flex flex-col gap-1.5 mt-1">
                  <div className="flex justify-between items-center px-1">
                    ...
                    <span className={`text-[11px] font-bold ${password.length > 0 ? 'opacity-100' : 'opacity-40'} transition-opacity`} style={{ color: strength.score > 0 ? '' : '#6B7280' }}>
                  ...
                  <p className="text-[10px] text-[#6B7280] text-left px-1 mt-0.5">
          ...
          <div className="text-center -mt-2">
            <a
              className="text-xs sm:text-sm font-semibold text-[#2E7D32] hover:text-[#43A047] transition-colors inline-flex items-center gap-2"
              href="/login"
            >
      ...
      {/* Footer Details */}
      <footer className="w-full py-4 px-6 flex flex-col md:flex-row justify-between items-center gap-3 bg-[#F8FAF7] border-t border-[#D8E3DA]/20 text-center md:text-left">
        <div className="text-xs font-bold text-[#1B1D1B]/80">
          © {new Date().getFullYear()} PoultraScan AI. All rights reserved.
        </div>
        <div className="flex flex-wrap justify-center gap-4">
          <a className="text-xs text-[#6B7280] hover:text-[#2E7D32] transition-colors" href="#">Privacy Policy</a>
          <a className="text-xs text-[#6B7280] hover:text-[#2E7D32] transition-colors" href="#">Terms of Service</a>
          <a className="text-xs text-[#6B7280] hover:text-[#2E7D32] transition-colors" href="#">Contact Support</a>
        </div>
      </footer>
