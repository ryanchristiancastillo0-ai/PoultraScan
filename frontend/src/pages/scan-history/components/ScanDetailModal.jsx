  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative w-full max-w-2xl max-h-[92vh] sm:max-h-[90vh] overflow-y-auto no-scrollbar bg-white rounded-t-2xl sm:rounded-2xl border border-[#E5E7EB] shadow-xl animate-modal-in">
        {/* Header strip */}
        <div className="sticky top-0 z-10 rounded-t-2xl bg-[#2E7D32] px-4 py-4 sm:px-6 sm:py-5 flex items-start justify-between gap-3 sm:gap-4">
        ...
        <div className="p-4 sm:p-6 flex flex-col gap-4 sm:gap-5">
          {/* Image */}
          <div className="relative w-full aspect-video bg-[#F8FAF7] rounded-xl overflow-hidden border border-[#E5E7EB] flex items-center justify-center">
            {src ? (
          ...
          <div className="flex flex-wrap items-center gap-2.5 sm:gap-3 justify-between">
            ...
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#F8FAF7] border border-[#E5E7EB]">
              <MdAgriculture className="text-[#6B7280] text-sm" />
              <span className="text-xs font-semibold text-[#1B1D1B]">
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            <div className="p-3 sm:p-4 rounded-xl bg-[#F8FAF7] border border-[#E5E7EB]">
              <p className="text-[10px] text-[#6B7280] uppercase tracking-wider font-semibold mb-1">Started</p>
              <p className="text-sm font-bold text-[#1B1D1B]">{date}</p>
            ...
            <div className="p-3 sm:p-4 rounded-xl bg-[#F8FAF7] border border-[#E5E7EB]">
              ...
              <p className="text-sm font-bold text-[#1B1D1B]">{scan.finished_at ? finishedDate : '--'}</p>
          ...
          {/* Remarks */}
          {scan.remarks && (
            <div className="flex flex-col gap-2 p-4 rounded-xl bg-[#E8F5E9] border border-[#E8F5E9]">
              <div className="flex items-center gap-2">
                <MdOutlineNotes className="text-[#2E7D32] text-base" />
                <span className="text-xs font-bold uppercase tracking-wider text-[#1B1D1B]">Remarks</span>
              </div>
          ...
          <div className="flex flex-col gap-2">
            <span className="text-xs font-bold uppercase tracking-wider text-[#1B1D1B]">
              Chicken-by-Chicken Results
            </span>
            {loadingChickens ? (
              <div className="py-6 flex justify-center">
                <div className="w-6 h-6 rounded-full border-2 border-[#2E7D32]/20 border-l-[#2E7D32] animate-spin" />
              </div>
            ) : chickens.length === 0 ? (
        ...
        <div className="px-4 pb-4 sm:px-6 sm:pb-6">
          <button
            onClick={onClose}
            className="w-full py-3 rounded-lg bg-[#2E7D32] text-white text-sm font-semibold hover:bg-[#276C2A] transition-all active:scale-95"
          >
            Close
          </button>
        </div>
